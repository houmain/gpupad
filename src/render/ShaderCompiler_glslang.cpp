...
namespace ShaderCompiler {
        ...
        void patchHLSLGlobalUniformBindingSet(glslang::TShader &shader)
        {
            ...
            struct Traverser : glslang::TLiveTraverser
            {
                ...
                void visitSymbol(glslang::TIntermSymbol *base) override
                {
                    if (base->getQualifier().isUniformOrBuffer()
                        ...
                        auto &qualifier =
                            base->getWritableType().getQualifier();
            ...
            if (auto intermediate = shader.getIntermediate())
                if (auto root = intermediate->getTreeRoot()) {
                    Traverser traverser(*intermediate, false, false, false,
                        true);
        ...
        ItemId programItemId, MessagePtrSet &messages)
    {
        ...
        for (const auto &input : inputs) {
            ...
            auto &shader = *shaders.emplace_back(createShader(session,
                defaultVersion, input.shaderType, input.sources,
                input.fileNames, input.entryPoint));

            auto includer = Includer(input.includePaths);
            if (!shader.parse(GetDefaultResources(), defaultVersion,
                    defaultProfile, false, forwardCompatible,
        ...
        MessagePtrSet &messages)
    {
        ...
        if (!shader->preprocess(GetResources(), defaultVersion, defaultProfile,
                false, forwardCompatible, requestedMessages, &string,
                includer)) {
            parseGLSLangErrors(QString::fromUtf8(shader->getInfoLog()),
    ...
    try {
        ...
        auto reflection = Reflection(spirv);
        ...
        MessagePtrSet &messages)
    {
        ...
        auto includer = Includer(includePaths);
        if (!shader->parse(GetDefaultResources(), defaultVersion,
                defaultProfile, false, forwardCompatible, requestedMessages,
                includer)) {
            parseGLSLangErrors(QString::fromUtf8(shader->getInfoLog()),
                messages, itemId, fileNames);
            return {};
